(() => {
  ...
  function drawAnchorStar(star, now, sparkBoost = 0) {
    ...
    const haloRadius = coreRadius * (4.8 + twinkleEnvelope * (4.2 + radiusVariation) + sparkBoost * 6.5);
    const halo = ctx.createRadialGradient(x, y, 0, x, y, haloRadius);
    halo.addColorStop(0, star.color);
    halo.addColorStop(.16, star.color);
    halo.addColorStop(.46, 'rgba(168,205,255,.28)');
    ...
    ctx.fill();
    ctx.globalAlpha = .38 + twinkleEnvelope * .68 + sparkBoost * .32;
    ctx.fillStyle = star.color;
    ctx.beginPath();
    ctx.arc(x, y, coreRadius * (.58 + twinkleEnvelope * .24 + sparkBoost * .28), 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  ...
  function spawnMeteor() {
    ...
    meteors.push({
      ...
      length: random(width * .035, width * .065),
      lineWidth: random(.42, .82),
      life: 0,
      maxLife: random(.62, .88),
      color: pick(['#eaf5ff', '#bfdcff', '#fff0d7'])
    });
  ...
  function drawMeteors(dt) {
    for (let index = meteors.length - 1; index >= 0; index -= 1) {
      ...
      const fade = Math.sin(progress * Math.PI) * .58;
      const trail = ctx.createLinearGradient(tailX, tailY, meteor.x, meteor.y);
      trail.addColorStop(0, 'rgba(255,255,255,0)');
      trail.addColorStop(.82, meteor.color);
